...
public class TooManyShapes {
	...
	public static void main(String[] args) {
		...
		int sidesAsInt = Integer.parseInt(sides);
		//2. Ask the user how many shapes they want
		String number = JOptionPane.showInputDialog("How many shapes do you want?");
		int numberAsInt = Integer.parseInt(number);
		//3. Call canMakeShape() and save what is returned into a variable
		boolean x = canMakeShape(sidesAsInt);
		
		//4. If the shape CAN be drawn
			if (x) {
				int a = calculateTurnAngle(sidesAsInt);
				drawPolygons(sidesAsInt, numberAsInt, a);
			}
